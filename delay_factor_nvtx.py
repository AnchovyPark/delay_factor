...
class DelayFactorExtractor:
    ...
    def benchmark_memory_read(self, bytes_targets: List[float]) -> Dict[float, float]:
        """
    ...
    def extract_delay_factors_separate(self, 
                            ...
                            bytes_range: Tuple[float, float] = (1e3, 1e10)) -> Dict[str, Dict[str, Dict[str, float]]]:
        """
        Extract delay factors with READ and WRITE measured separately.
        ...
        Returns:
            Dictionary in the format with separate read/write delay factors
        """
        print(f"\\nExtracting delay factors (READ/WRITE separate) for GPU: {self.gpu_name}")
        print(f"FLOPS range: {flops_range[0]:.0e} to {flops_range[1]:.0e}")
        print(f"Bytes range: {bytes_range[0]:.0e} to {bytes_range[1]:.0e}")
        ...
        # Benchmark Memory READ and WRITE separately
        read_delay_factors = self.benchmark_memory_read(bytes_targets)
        write_delay_factors = self.benchmark_memory_write(bytes_targets)
        ...
        result = {
            self._normalize_gpu_name(): {
                ...
                "read_delay_factors": {
                    str(int(bytes_val)): delay_factor 
                    for bytes_val, delay_factor in read_delay_factors.items()
                },
                "write_delay_factors": {
                    str(int(bytes_val)): delay_factor 
                    for bytes_val, delay_factor in write_delay_factors.items()
                }
            }
        }
...
def main():
    """Main function to extract delay factors with separate READ/WRITE measurement."""
    print("="*80)
    print("GPU Delay Factor Extractor - Separate READ/WRITE Mode")
    print("="*80)
    ...
    try:
        ...
        # Extract delay factors with READ/WRITE measured separately
        print("\\nStarting benchmark with separate READ/WRITE measurement...")
        delay_factors = extractor.extract_delay_factors_separate(
        ...
        read_factors = delay_factors[gpu_name]["read_delay_factors"]
        write_factors = delay_factors[gpu_name]["write_delay_factors"]
        ...
        print(f"READ delay factors: {len(read_factors)} points")
        print(f"WRITE delay factors: {len(write_factors)} points")
        print(f"FLOPS range: {min(flops_factors.values()):.2f} - {max(flops_factors.values()):.2f}")
        print(f"READ range: {min(read_factors.values()):.2f} - {max(read_factors.values()):.2f}")
        print(f"WRITE range: {min(write_factors.values()):.2f} - {max(write_factors.values()):.2f}")
