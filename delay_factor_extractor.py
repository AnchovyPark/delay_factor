...
class DelayFactorExtractor:
    ...
    def benchmark_memory_read(self, bytes_targets: List[float]) -> Dict[float, float]:
        ...
        for target_bytes in bytes_targets:
            ...
            try:
                ...
                for _ in range(self.warmup_iterations):
                    _ = torch.sum(src_tensor)  # Read all elements
                torch.cuda.synchronize()
                ...
                for _ in range(self.measure_iterations):
                    start_time = time.perf_counter()
                    result = torch.sum(src_tensor)  # Forces reading all elements
                    torch.cuda.synchronize()
                    end_time = time.perf_counter()
                    times.append(end_time - start_time)
                ...
                print(f"  READ delay factor: {delay_factor:.2f}")
    ...
    def benchmark_memory_write(self, bytes_targets: List[float]) -> Dict[float, float]:
        ...
        for target_bytes in bytes_targets:
            ...
            try:
                ...
                for _ in range(self.warmup_iterations):
                    dst_tensor.fill_(1.0)  # Write same value to all elements
                torch.cuda.synchronize()
                ...
                for _ in range(self.measure_iterations):
                    start_time = time.perf_counter()
                    dst_tensor.fill_(1.0)  # Forces writing to all elements
                    torch.cuda.synchronize()
                    end_time = time.perf_counter()
                    times.append(end_time - start_time)
                ...
                print(f"  Theoretical bandwidth: {theoretical_bandwidth:.2f} GB/s")
                print(f"  WRITE delay factor: {delay_factor:.2f}")
    ...
    def quick_benchmark_separate(self) -> Dict[str, Dict[str, Dict[str, float]]]:
        """
        Run a quick benchmark with READ/WRITE measured separately.
        
        Returns:
            Delay factors dictionary with separate read/write factors
        """
        return self.extract_delay_factors_separate(
            flops_range=(1e3, 1e13),    # 1K to 10T FLOPs (더 넓은 범위)
            bytes_range=(1e3, 1e10),    # 1K to 10G bytes (더 넓은 범위)  
            num_points=12               # 더 많은 측정 포인트
        )
